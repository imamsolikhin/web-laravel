...
class ReportPatientController extends Controller {
    ...
    {
        static::$module = 'Laporan Data Pasien';
        static::$module_alias = 'Laporan Data Pasien';
        static::$auth = 'report-patient';
        static::$path = route('clinic.index','report-patient');
        static::$data = route('clinic.list','report-patient');
        static::$delete = route('clinic.delete',['report-patient','']);
        static::$controller = getControllerName("Clinic", "VisitPatient");
        static::$resource = getResourceName("Clinic", "VisitPatient");
    ...
    public static function index() {
      ...
      // return view('clinic.visit-patient',$data);
    ...
    public static function list($request) {
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Clinic","Laporan Data Pasien",sess_company('id'),null, $numb=5);
        }
        if ($request->name){
