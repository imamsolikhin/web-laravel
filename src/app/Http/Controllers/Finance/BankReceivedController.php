...
class BankReceivedController extends Controller {
    ...
    {
        static::$module = 'Bank Received';
        static::$module_alias = 'Bank Received';
        static::$auth = 'bank-received';
        static::$path = route('finance.index','bank-received');
        static::$data = route('finance.list','bank-received');
        static::$delete = route('finance.delete',['bank-received','']);
        static::$controller = getControllerName("Finance", "Bank Received");
        static::$resource = getResourceName("Finance", "Bank Received");
    ...
    public static function index() {
      ...
      return view('finance.bank-received',$data);
    }
    ...
    public static function save($request) {
        ...
        if ($validator->fails()) return redirect()->route('finance.index',static::$auth)->with('notif_danger', 'New '.static::$module_alias.' '. $request->name .' can not be save!');

        $module = static::$controller::execute($request);
        return redirect()->route('finance.index',static::$auth)->with('notif_success', 'New '.static::$module_alias.' '. $request->name .' has been added successfully!');
    }

    public static function update($id, $request) {
        self::init();
        $validator = static::$controller::validation($request,'update');
        if ($validator->fails()) return redirect()->route('finance.index',static::$auth)->with('notif_danger', 'Update '.static::$module_alias.' '. $request->name .' can not be udate!');

        $data = static::$resource::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('finance.index',static::$auth)->with('notif_danger', 'Data '. $id .' not found!');

        $module = static::$controller::execute($request,$data);
        return redirect()->route('finance.index',static::$auth)->with('notif_success', static::$module_alias.' '. $data->name .' has been update successfully!');
    }
    ...
    public static function list($request) {
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Product","Company",sess_company('id'),null, $numb=5);
        }
        if ($request->name){
