...
class SalesReturnController extends Controller {
    ...
    public static function index() {
      ...
      return view('Product.sales-return',$data);
    }
    ...
    public static function save($request) {
        ...
        $validator = getControllerName("Product", "Closing")::validation($request);
        if ($validator->fails()) return redirect()->route('Product.index','Closing')->with('notif_danger', 'New Closing '. $request->full_name .' can not be save!');

        $module = getControllerName("Product", "Closing")::execute($request);
        return redirect()->route('Product.index','Closing')->with('notif_success', 'New Closing '. $request->full_name .' has been added successfully!');
    }

    public static function update($id, $request) {

        $validator = getControllerName("Product", "Closing")::validation($request,'update');
        if ($validator->fails()) return redirect()->route('Product.index','Closing')->with('notif_danger', 'New Closing '. $request->full_name .' can not be udate!');

        $data = Customer::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('Product.index','Closing')->with('notif_danger', 'Data '. $id .' not found!');

        $module = getControllerName("Product", "Closing")::execute($request,$data);
        if($request->file_closing){
          ...
          $module = getControllerName("Product", "Closing")::execute($request,$module);
        }
        $datas = Visitor::find(str_replace('%20', ' ', $id));
        if ($datas){
          getControllerName("Product", "Interaksi")::execute($request,$datas);
        }
        $datas = Followup::find(str_replace('%20', ' ', $id));
        if ($datas){
          getControllerName("Product", "Followup")::execute($request,$datas);
        }

        return redirect()->route('Product.index','Closing')->with('notif_success', 'Closing '. $data->full_name .' has been update successfully!');
    }
    ...
    public static function list($request) {
        ...
        return DataTables::of($result)
          ...
          ->addColumn('action', function($module) {
              ...
              $delete = '<a data-href="' . route('Product.delete',['closing',$module->id]) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Product","Visitor",sess_company('id'), "RSV".date("ymd", time()), $numb=5);
        }
