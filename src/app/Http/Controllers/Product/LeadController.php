...
class LeadController extends Controller {
    ...
    public static function save($request) {
        ...
        $validator = getControllerName("Product", "Lead")::validation($request);
        if ($validator->fails()) return redirect()->route('product.index','Lead')->with('notif_danger', 'New Lead '. $request->full_name .' can not be save!');

        $module = getControllerName("Product", "Lead")::execute($request);
        return redirect()->route('product.index','Lead')->with('notif_success', 'New Lead '. $request->full_name .' has been added successfully!');
    }

    public static function update($id, $request) {

        $validator = getControllerName("Product", "Lead")::validation($request,'update');
        if ($validator->fails()) return redirect()->route('product.index','Lead')->with('notif_danger', 'New Lead '. $request->full_name .' can not be udate!');

        $data = Customer::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('product.index','Lead')->with('notif_danger', 'Data '. $id .' not found!');

        $module = getControllerName("Product", "Lead")::execute($request,$data);

        $data = Visitor::find(str_replace('%20', ' ', $id));
        getControllerName("Product", "Interaksi")::execute($request,$data);

        return redirect()->route('product.index','Lead')->with('notif_success', 'Lead '. $data->full_name .' has been update successfully!');
    }
    ...
    public static function list($request) {
        ...
        return DataTables::of($result)
          ...
          ->addColumn('action', function($module) {
              ...
              $delete = '<a data-href="' . route('product.delete',['lead',$module->id]) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
