...
class FollowupController extends Controller {
    ...
    public static function save($request) {
        ...
        $validator = getControllerName("Product", "Followup")::validation($request);
        if ($validator->fails()) return redirect()->route('product.index','Followup')->with('notif_danger', 'New Followup '. $request->full_name .' can not be save!');

        $module = getControllerName("Product", "Followup")::execute($request);

        $visitor = new Visitor($module->getOriginal());
        $visitor->lock_status = 1;
        $visitor->followup_status = 1;
        $visitor->followup_date = currDate();
        getControllerName("Product", "Interaksi")::execute($request,$visitor);
        ...
        return redirect()->route('product.index','Followup')->with('notif_success', 'New Followup '. $request->full_name .' has been added successfully!');
    }

    public static function update($id, $request) {

        $validator = getControllerName("Product", "Followup")::validation($request,'update');
        if ($validator->fails()) return redirect()->route('product.index','Followup')->with('notif_danger', 'New Followup '. $request->full_name .' can not be udate!');

        $data = Followup::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('product.index','Followup')->with('notif_danger', 'Data '. $id .' not found!');
        ...
        return redirect()->route('product.index','Followup')->with('notif_success', 'Followup '. $data->full_name .' has been update successfully!');
    }
    ...
    public static function list($request) {
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Product","Followup",sess_company('id'), "FLW".date("ymd", time()), $numb=5);
        }
