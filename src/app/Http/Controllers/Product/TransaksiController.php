...
class TransaksiController extends Controller {
    ...
    public static function index() {
      ...
      return view('product.transaksi',$data);
    }

    public static function data($id) {
        $module = Customer::withoutGlobalScopes(['active'])->findOrFail($id);
    ...
    public static function save($request) {
        ...
        $validator = getControllerName("Product", "Transaksi")::validation($request);
        if ($validator->fails()) return redirect()->route('product.index','Transaksi')->with('notif_danger', 'New Transaksi '. $request->full_name .' can not be save!');

        $module = getControllerName("Product", "Transaksi")::execute($request);
        return redirect()->route('product.index','Transaksi')->with('notif_success', 'New Transaksi '. $request->full_name .' has been added successfully!');
    }

    public static function update($id, $request) {

        $validator = getControllerName("Product", "Transaksi")::validation($request,'update');
        if ($validator->fails()) return redirect()->route('product.index','Transaksi')->with('notif_danger', 'New Transaksi '. $request->full_name .' can not be udate!');

        $data = Customer::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('product.index','Transaksi')->with('notif_danger', 'Data '. $id .' not found!');

        $module = getControllerName("Product", "Transaksi")::execute($request,$data);

        $data = Visitor::find(str_replace('%20', ' ', $id));
        getControllerName("Product", "Interaksi")::execute($request,$data);

        return redirect()->route('product.index','Transaksi')->with('notif_success', 'Transaksi '. $data->full_name .' has been update successfully!');
    }
    ...
    public static function delete($id) {
        ...
        return redirect()->back()->with('notif_success', 'Transaksi '. $data->full_name .' has been deleted!');
    }
    ...
    public static function list($request) {
        if($request->from_date != '' && $request->from_date  != ''){
          $result = Customer::withoutGlobalScopes()
                    ->whereBetween('reservation_date', array($request->from_date, $request->to_date)) ;
        }else{
        	$result = Customer::withoutGlobalScopes()->where('status',0);
        }
        ...
        return DataTables::of($result)
          ...
          ->addColumn('action', function($module) {
              ...
              $delete = '<a data-href="' . route('product.delete',['transaksi',$module->id]) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Product","Visitor",sess_company('id'), "LAD".date("ymd", time()), $numb=5);
        }
