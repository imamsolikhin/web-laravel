...
class InteraksiController extends Controller {
    ...
    public static function save($request) {
        ...
        if ($validator->fails()) return redirect()->route('product.index','Interaksi')->with('notif_danger', 'New Interaksi '. $request->full_name .' can not be save!');

        $module = getControllerName("Product", "Interaksi")::execute($request);
        ...
        return redirect()->route('product.index','Interaksi')->with('notif_success', 'New Interaksi '. $request->full_name .' has been added successfully!');
    }

    public static function update($id, $request) {

        $validator = getControllerName("Product", "Interaksi")::validation($request,'update');
        if ($validator->fails()) return redirect()->route('product.index','Interaksi')->with('notif_danger', 'New Interaksi '. $request->full_name .' can not be udate!');
        ...
        $module = getControllerName("Product", "Interaksi")::execute($request,$data);
        ...
        return redirect()->route('product.index','Interaksi')->with('notif_success', 'Interaksi '. $data->full_name .' has been update successfully!');
    }
    ...
    public static function list($request) {
        ...
        return DataTables::of($result)
          ...
          ->addColumn('action', function($module) {
              ...
              $delete = '<a data-href="' . route('product.delete',['interaksi',$module->id]) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Product","Visitor",sess_company('id'), "VST".date("ymd", time()), $numb=5);
        }
        ...
        if ($request->except("status")) {
            ...
            $data->lock_status = to_bool($request->status);
        }
