...
class DeliveryReturnController extends Controller {
    ...
    {
        static::$module = 'DeliveryReturn';
        static::$module_alias = 'DeliveryReturn';
        static::$auth = 'delivery-order';
        static::$path = route('warehouse.index','delivery-order');
        static::$data = route('warehouse.list','delivery-order');
        static::$delete = route('warehouse.delete',['delivery-order','']);
        static::$controller = getControllerName("Warehouse", "DeliveryReturn");
        static::$resource = getResourceName("Warehouse", "DeliveryReturn");
    ...
    public static function index() {
      ...
      return view('warehouse.delivery-order',$data);
    }
    ...
    public static function save($request) {
        ...
        if ($validator->fails()) return redirect()->route('warehouse.index',static::$auth)->with('notif_danger', 'New '.static::$module_alias.' '. $request->name .' can not be save!');
    ...
    public static function update($id, $request) {
        ...
        if ($validator->fails()) return redirect()->route('warehouse.index',static::$auth)->with('notif_danger', 'Update '.static::$module_alias.' '. $request->name .' can not be udate!');
    ...
    public static function list($request) {
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Warehouse","DeliveryReturn",sess_company('id'),null, $numb=5);
        }
        if ($request->name){
