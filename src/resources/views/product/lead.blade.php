  ...
  <div class="{{ Metronic::printClasses('subheader-container', false) }} d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
    <div class="d-flex align-items-baseline flex-wrap mr-5">
      ...
      <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        ...
        <li class="breadcrumb-item">
          <a href="{{ route('product.index','lead') }}" class="text-muted">Lead</a>
        </li>
        <li class="breadcrumb-item">
...
<div class="modal fade" id="modal-form" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class="form" id="form-input" action="{{ route('product.save','lead') }}" method="POST">
              {!! csrf_field() !!}
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-8">
                            <select class="form-control select2" id="interaction" name="interaction" style="width: 100%;">
                              <option value="" selected>Chose Lead</option>
...
<script type="text/javascript">
  ...
  function show_data(id = "") {
      if (id !== "") {
          $.ajax({
              url: "{{ route('product.list','lead')}}/" + id,
              type: "GET",
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
              },
              success: function (response) {
                  $("#form-input").attr("action", "{{ route('product.update',['lead',''])}}/"+id);
        ...
        $("#form-input").attr("action", "{{ route('product.save','lead')}}");
          $('#form-input').trigger("reset");
          $('#method').val("POST");
