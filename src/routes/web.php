...
$router->group(['prefix' => 'reports', 'namespace' => 'Reports', 'middleware' => 'auth'], function () use ($router) {
    $router->group(['prefix' => '{table}'], function () use ($router) {
        $router->get('/{export}/view', [
            'as' => 'reports.view', 'uses' => 'ReportsController@view'
        ]);
...
$router->group(['prefix' => 'master', 'namespace' => 'Master', 'middleware' => 'auth'], function () use ($router) {
 ...
 $router->group(['prefix' => 'finance', 'namespace' => 'Finance'], function () use ($router) {
     $router->group(['prefix' => '{table}'], function () use ($router) {
         ...
         $router->post('list', [
             'as' => 'finance.list', 'uses' => 'FinanceController@list'
         ]);
         $router->get('list', [
             'as' => 'finance.list', 'uses' => 'FinanceController@list'
         ]);
         $router->get('data/{id}', [
             'as' => 'finance.data', 'uses' => 'FinanceController@data'
         ]);
         $router->post('save', [
             'as' => 'finance.save', 'uses' => 'FinanceController@save'
         ]);
         $router->post('update/{id}', [
             'as' => 'finance.update', 'uses' => 'FinanceController@update'
         ]);
         $router->delete('{id}', [
             'as' => 'finance.delete', 'uses' => 'FinanceController@delete'
         ]);
     });
 });
 ...
 $router->group(['prefix' => 'warehouse', 'namespace' => 'Warehouse'], function () use ($router) {
     $router->group(['prefix' => '{table}'], function () use ($router) {
         ...
         $router->delete('{id}', [
             'as' => 'warehouse.delete', 'uses' => 'WarehouseController@delete'
         ]);
     });
 });
