  ...
  <div class="{{ Metronic::printClasses('subheader-container', false) }} d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
    <div class="d-flex align-items-baseline flex-wrap mr-5">
      ...
      <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        ...
        <li class="breadcrumb-item">
          <a href="{{ route('master.itemprice.index') }}" class="text-muted">Item Price</a>
        </li>
        <li class="breadcrumb-item">
...
<div class="card card-custom">
  <div class="card-header bg-danger flex-wrap border-1 pt-1 pb-0 mb-2" style="min-height: 0;">
    <div class="card-title pt-1 pb-1">
      <h3 class="card-label font-weight-bolder text-white">Item Price
        <div class="text-muted pt-2 font-size-lg">show Datatable from table Item Price</div>
      </h3>
    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-danger pt-3 pb-3">
                <h5 class="modal-title text-white bold" id="modal">New Item Price</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <form class="form" id="form-input" action="{{ route('master.itemprice.store') }}" method="POST">
              {!! csrf_field() !!}
...
<script type="text/javascript">
  ...
  var table = $('#datatable').dataTable({
    ...
    ajax: {
      method: 'POST',
      url : '{{ route('master.itemprice.data') }}',
      headers: {
  ...
  function show_data(id = "") {
      if (id !== "") {
          $.ajax({
              url: "{{ route('master.itemprice.store')}}/" + id,
              type: "GET",
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
              },
              success: function (response) {
                  $("#form-input").attr("action", "{{ route('master.itemprice.update','')}}/"+id);
          ...
          $("#form-input").attr("action", "{{ route('master.itemprice.store')}}");
          $('#form-input').trigger("reset");
          $('#method').val("POST");
