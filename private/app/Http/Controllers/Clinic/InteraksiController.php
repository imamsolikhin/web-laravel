...
class InteraksiController extends Controller {
    ...
    public function index() {
      ...
      $data["confirmation_list"] = Confirmation::where('ActiveStatus',1)->get();
        return view('clinic.interaksi',$data);
    }
    ...
    public function store(Request $request) {
        ...
        if ($validator->fails()) return redirect()->route('clinic.interaksi.index')->with('notif_danger', 'New Interaksi '. $request->Name .' can not be save!');
        ...
        return redirect()->route('clinic.interaksi.index')->with('notif_success', 'New Interaksi '. $request->Name .' has been added successfully!');
    }

    public function update($id, Request $request) {

        $data = Interaksi::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('clinic.interaksi.index')->with('notif_danger', 'Data '. $id .' not found!');

        $validator = $this->validation($request, 'update');
        if ($validator->fails()) return redirect()->route('clinic.interaksi.index')->with('notif_danger', 'Interaksi '. $data->Name .' can not be update!');
        ...
        return redirect()->route('clinic.interaksi.index')->with('notif_success', 'Interaksi '. $data->Name .' has been update successfully!');
    }
    ...
    public function getData(Request $request) {
        ...
        return DataTables::of($result)
                        ...
                        ->addColumn('action', function($interaksi) {
                            $detail = '<a href="' . route('clinic.interaksi.show', $interaksi->Code) . '" class="btn btn-icon btn-light btn-hover-success btn-sm" data-toggle="tooltip" data-placement="top" title="Detail">
                            ...
                            $delete = '<a data-href="' . route('clinic.interaksi.destroy', $interaksi->Code) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
     ...
     public function save($request, $data = null) {
        ...
        if ($request->Code) {
            ...
            $data->Code = strtoupper(generateRandomString(5));
        }
