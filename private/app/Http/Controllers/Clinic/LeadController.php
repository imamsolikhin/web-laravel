...
class LeadController extends Controller {
    ...
    public function store(Request $request) {
        ...
        if ($validator->fails()) return redirect()->route('clinic.lead.index')->with('notif_danger', 'New Lead '. $request->Name .' can not be save!');

        $lead = $this->save($request);

        return redirect()->route('clinic.lead.index')->with('notif_success', 'New Lead '. $request->Name .' has been added successfully!');
    }

    public function update($id, Request $request) {

        $data = Lead::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('clinic.lead.index')->with('notif_danger', 'Data '. $id .' not found!');

        $validator = $this->validation($request, 'update');
        if ($validator->fails()) return redirect()->route('clinic.lead.index')->with('notif_danger', 'Lead '. $data->Name .' can not be update!');

        $lead = $this->save($request,$data);

        return redirect()->route('clinic.lead.index')->with('notif_success', 'Lead '. $data->Name .' has been update successfully!');
    }
    ...
    public function getData(Request $request) {
        ...
        return DataTables::of($result)
                        ...
                        ->addColumn('action', function($lead) {
                            $detail = '<a href="' . route('clinic.lead.show', $lead->Code) . '" class="btn btn-icon btn-light btn-hover-success btn-sm" data-toggle="tooltip" data-placement="top" title="Detail">
                            ...
                            $delete = '<a data-href="' . route('clinic.lead.destroy', $lead->Code) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
