...
class LeadController extends Controller {
    ...
    public static function save($request) {
        ...
        if ($validator->fails()) return redirect()->route('clinic.index','lead')->with('notif_danger', 'New Lead '. $request->Name .' can not be save!');

        $lead = getControllerName("Clinic", "Lead")::execute($request);

        return redirect()->route('clinic.index','lead')->with('notif_success', 'New Lead '. $request->Name .' has been added successfully!');
    }

    public static function update($id, $request) {

        $validator = getControllerName("Clinic", "Lead")::validation($request);
        if ($validator->fails()) return redirect()->route('clinic.index','lead')->with('notif_danger', 'New Lead '. $request->Name .' can not be Update!');
