...
class ClosingController extends Controller {
    ...
    public function store(Request $request) {
        ...
        if ($validator->fails()) return redirect()->route('clinic.closing.index')->with('notif_danger', 'New Closing '. $request->Name .' can not be save!');

        $closing = $this->save($request);

        return redirect()->route('clinic.closing.index')->with('notif_success', 'New Closing '. $request->Name .' has been added successfully!');
    }

    public function update($id, Request $request) {

        $data = Closing::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('clinic.closing.index')->with('notif_danger', 'Data '. $id .' not found!');

        $validator = $this->validation($request, 'update');
        if ($validator->fails()) return redirect()->route('clinic.closing.index')->with('notif_danger', 'Closing '. $data->Name .' can not be update!');

        $closing = $this->save($request,$data);

        $interaksi = Interaksi::where('Phone', '=', $closing->Phone)->update(['LockStatus'=>1]);

        return redirect()->route('clinic.closing.index')->with('notif_success', 'Closing '. $data->Name .' has been update successfully!');
    }
    ...
    public function getData(Request $request) {
        ...
        return DataTables::of($result)
                        ...
                        ->addColumn('active', function($closing) {
                            ...
                            $status =  $closing->Status ? '<span class="label font-weight-bold label-lg  label-light-info label-inline">Kunjungan</span>' : '<span class="label font-weight-bold label-lg  label-light-warning label-inline">Reservasi</span>';
                        ...
                        ->addColumn('action', function($closing) {
                            $detail = '<a href="' . route('clinic.closing.show', $closing->Code) . '" class="btn btn-icon btn-light btn-hover-success btn-sm" data-toggle="tooltip" data-placement="top" title="Detail">
                            ...
                            $delete = '<a data-href="' . route('clinic.closing.destroy', $closing->Code) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
