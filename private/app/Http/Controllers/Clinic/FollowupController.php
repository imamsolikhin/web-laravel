...
class FollowupController extends Controller {
    ...
    public function store(Request $request) {
        ...
        if ($validator->fails()) return redirect()->route('clinic.followup.index')->with('notif_danger', 'New Followup '. $request->Name .' can not be save!');
        ...
        return redirect()->route('clinic.followup.index')->with('notif_success', 'New Followup '. $request->Name .' has been added successfully!');
    }

    public function update($id, Request $request) {

        $data = Followup::find(str_replace('%20', ' ', $id));
        if (!$data) return redirect()->route('clinic.followup.index')->with('notif_danger', 'Data '. $id .' not found!');

        $validator = $this->validation($request, 'update');
        if ($validator->fails()) return redirect()->route('clinic.followup.index')->with('notif_danger', 'Followup '. $data->Name .' can not be update!');
        ...
        return redirect()->route('clinic.followup.index')->with('notif_success', 'Followup '. $data->Name .' has been update successfully!');
    }
    ...
    public function getData(Request $request) {
        ...
        return DataTables::of($result)
                        ...
                        ->addColumn('action', function($followup) {
                            $detail = '<a href="' . route('clinic.followup.show', $followup->Code) . '" class="btn btn-icon btn-light btn-hover-success btn-sm" data-toggle="tooltip" data-placement="top" title="Detail">
                            ...
                            $delete = '<a data-href="' . route('clinic.followup.destroy', $followup->Code) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
