...
class BankController extends Controller {
    ...
    {
        static::$module = 'Bank';
        static::$module_alias = 'Bank';
        static::$auth = 'bank';
        static::$path = route('master.index','bank');
        static::$data = route('master.list','bank');
        static::$delete = route('master.delete',['bank','']);
        static::$controller = getControllerName("Master", "Bank");
        static::$resource = getResourceName("Master", "Bank");
    ...
    public static function list($request) {
     ...
     public static function execute($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Master","Bank",sess_company('id'),null, $numb=5);
        }
        if ($request->name){
