...
class AdvertiseController extends Controller {
    ...
    public function index() {
        ...
        $data['auth'] = 'advertise';
        $data['path'] = route('master.advertise.index');
        $data['data'] = route('master.advertise.data');
        return view('master.advertise',$data);
    }

    public function show($id) {
        $module = Advertise::select("*")->where("id","=",$id)->first();
    ...
    public function getData() {
        ...
        return DataTables::of($result)
                ...
                ->addColumn('action', function($module) {
                    ...
                    $delete = '<a data-href="' . route('master.advertise.destroy', $module->id) . '" class="btn btn-icon btn-light btn-hover-danger btn-sm" "data-toggle="tooltip" data-placement="top" title="Delete" data-toggle="modal" data-target="#confirm-delete-modal">
     ...
     public function validation($request, $type = null) {
         ...
         if (is_null($type)) {
             $rules = array_merge($rules, ['id' => 'required|max:250|unique:mst_advertise,id']);
         }
     ...
     public function save($request, $data = null) {
        ...
        if ($request->id) {
            ...
            $data->id = generadeCode("Master","Advertise",sess_company('id'), "CTY", $numb=5);
        }
        if ($request->name){
